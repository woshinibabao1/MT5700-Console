...
### 修复 - 「设备控制」的三项设置（参数个数、值域、文案都按手册纠偏）

同一次只读复核里核对真机（MT5700M-CN / V200R001C20B025）：

```text
AT^LEDSWITCH?    → ^LEDSWITCH:1         AT^LEDSWITCH=?    → ^LEDSWITCH:(0-1)
AT^TDPMCFG?      → ^TDPMCFG: 1,0,0,0    AT^TDPMCFG=?      → ^TDPMCFG ((0-1),(0-1),(0-1),(0-1)
                                                             byte[0]:pcie / aothers reserve
AT^TDPCIELANCFG? → ^TDPCIELANCFG: 1     AT^TDPCIELANCFG=? → ^TDPCIELANCFG: (0,1)
```

这一卡三项都写模组掉电保存区，之前有三处与手册不符：

1. **「电源管理」少发参数**：原实现下发 `AT^TDPMCFG=1`，而手册 11.16 的命令是
   `AT^TDPMCFG=<mode>,<mode>,<mode>,<mode>`，**只有 byte[0] 有定义（pcie）**，其余
   三位是保留位 —— 少发的位行为未定义。现按 `1|0,<读回的保留位原样回写>` 拼满四参。
   该项同时按手册正名为「**PCIe 控制器**」：它控制的是模组内部 PCIe 控制器供电，
   与原来写的「开启后模组在无业务时进入低功耗」无关。
2. **「网卡速率」的值域和文案都不是本模组的**：下拉原本是
   `自动协商 / 1000Mbps 全双工 / 100Mbps 全双工 / 10Mbps 全双工`（0/1/2/3），那是
   **别的模组型号**的定义。手册 11.19 的语义是「选 PCIe 网口所接的 PHY 型号」
   （1=RTL8111 1G、2=RTL8125 2.5G），厂家 HiGoROS webui 也这么标；而本机固件 `=?`
   实报值域只有 **(0,1)**，原来的 2 / 3 发下去会被模组拒绝。
   现改为**按 `=?` 实报生成选项**（新增 `Parse.parseNicRange`，枚举式 `(0,1)` 与范围式
   `(0-1)` 都吃，解析不出就返回 null），读不到就**禁用下拉**，绝不把模组不认的值发下去。
   项名相应改为「网卡 PHY 型号」。
3. **交互与拨号页统一**：三项改为「暂存 → 保存并应用」，不再拨一下开关就写模组、
   改一次下拉就弹一个「是否立即重启模组」（连改三项要弹三次）。卡内补「立即重启模组」
   入口（LED 与网卡 PHY 需重启模组才生效）。

卡片同时写明：本机 5G 模组以 USB 方式供网（eth2 = CDC-NCM），SoC 侧 PCIe 上只有
mt7996e WiFi、**没有 PCIe 网卡** —— 「PCIe 控制器」与「网卡 PHY 型号」对上网速率
都没有影响。

新增 `tests/device-control-contract.test.js`（32 项）：用真机原文固定 `parseNicRange`
的解析结果，并断言四条防回归约定 —— 不得再出现「1000Mbps 全双工」这类外型号文案、
不得再出现只发 1 个参数的 `AT^TDPMCFG=`、三项必须走暂存、暂存项的 run 失败必须 reject
（否则界面报「已应用」而模组其实没改）。
