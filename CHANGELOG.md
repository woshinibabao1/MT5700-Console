...
### 变更 - SIM PIN / SIM 卡信息页重做

按设计稿把原来「4 个按钮 + 弹窗」改成两张卡片：

- **PIN 码管理**：卡片头徽章（PIN 已启用 / 未启用）+ 状态提示条 + 内联密码框 +
  按钮组 + 危险区（PUK 解锁单独红框隔离 + 二次确认 —— PUK 只有 10 次机会）。
- **SIM 卡信息**：头徽章（已就绪 / 等待 PIN / 已锁定）+ 状态 / ICCID / IMSI / 运营商。
