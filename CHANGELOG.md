...
> **长短信合并后的逻辑消息数**，两个数字并列却没有任何说明，看起来就像"短信丢了"。

### 修复 - 短信条数口径不一致，易被误读为"短信丢失"

**问题**：

- 顶部「存储：11 / 50」来自 `AT+CPMS?`，是 **SIM 实际占用的槽位数**
  （长短信的每个分片各占 1 个槽位）；
- 左侧列表经 `mergeConcatenated()` 合并后显示 **6 条**（逻辑消息数）；
- **两个数字都是对的**，但 UI 没说清关系 → 容易被当成"少了 5 条短信"。

**改动**（单文件：`htdocs/luci-static/resources/view/at-webserver/sms_center.js`）：

| 位置 | 改前 | 改后 |
| --- | --- | --- |
| 顶部存储行 `renderStorage()` | `存储：11 / 50` | `存储：11 / 50 · 6 条消息`（两值不等时才补注；相等时不啰嗦） |
| 长短信气泡 | 仅**缺片**时标注 | **凡合并过的都标注**「长短信已合并 N/M 段」，缺片再追加「（部分缺失）」 |
| 单条删除提示 | `删除成功` | 长短信显示「已删除该长短信的全部 N 个分片」 |
| 批量删除提示 | `成功删除 N 条短信` | `成功删除 N 条消息（共 M 个分片）`（M > N 时） |
| `buildContacts()` | — | 末尾追加 `renderStorage()`，合并完成后同步刷新存储行 |

新增 `countReceivedMessages()`：统计**合并后**的接收类消息数
（本地缓存的已发消息不占模组存储，不计入）。

**边界处理**：

- `AT+CPMS?` 尚未返回时存储行显示 `存储：—`（原先会短暂显示 `存储：0 / 0`）；
- 没有长短信时（`used === 消息数`）不追加后缀，界面保持简洁。

**验证**：以模组内**真实的 11 条 SMS** 数据跑合并 + 计数 + 文案断言，全部通过
—— 11 条 → 6 条消息；`0x52` 组 2/2、`0xCE` 组 2/2、`0x4B` 组 4/5（缺片）；
存储行文案 `存储：11 / 50 · 6 条消息`；无长短信时输出 `存储：6 / 50`。

### 已知问题 - 短信**发送**仍未跑通（本轮真机复测结论）

对发送路径做了完整真机复测，**短信依然发不出去**，且证据指向**模组/SIM 侧**而非应用层：

- **收件箱取证**：`AT+CPMS?` = `"SM",11,50`，11 条全部是运营商主动推送的
  营销/验证码短信（星巴克权益、移动犀购、咪咕悦看、10086 验证码），
  **没有任何一条是 LLCX → 10086 的回复**，09-12 / 09-13 零新增；
- **官方 WebUI 是"假成功"**：`inotdream/mt5700webui-openwrt-server`（自官方固件提取）
  发出 `AT+CMGS=<len>\r<PDU>` 后，只判断 WebSocket 请求是否送达就提示「发送成功」，
  **从不检查 `+CMGS:`**；而模组对 `AT+CMGS=N` 的应答恰好就是 `OK`（拒绝）→ 被误判为成功；
- **真机字节序列实测**：`AT+CMGS=15` 以**单 `\r`** 结尾 → 模组回 `OK` 拒绝、不进输入态；
  以 **`\r\n`（CR+LF）** 结尾 → 模组**只回显、进入输入态**；
  但即使成功进入输入态，最终仍收不到 `+CMGS:`；
- **`AT+CMGW`（存草稿，不发送）同样进不了输入态**，排除"仅发送权限受限"；
- **`AT^SIMSQ?` = `1,11`**；手册 p170 定义 **11 = 仅网络可接入、
  12 = 短信和电话本可接入**。本机稳定停在 11（`AT+CFUN=0/1` 复位两次无效）。
  但 SM 存储可读、能正常收短信 → **存储可用 ≠ 短信提交可用**。

**下一步**：需验证「配合 QModem（`ubus-at-daemon` + `sms-tool_q`）可以发短信」这一说法
在本机上是否成立，以判断是否存在我们尚未复现的初始化步骤。

### 说明 - 长短信合并是**预期行为**，不是缺陷

11 条原始 SMS 经 `mergeConcatenated()` 按「发件人 + UDH concat ref」合并为 6 条消息，
映射：`0x52`(2 片)→星巴克、`0x4B`(4/5 片)→移动犀购、`0xCE`(2 片)→咪咕悦看，
外加 3 条独立短信。此前「WebUI 只能看到 6 条」正源于此。

## [1.0.1] - 2026-09-13

> 紧急补丁：v1.0 的短信发送从未跑通。沿用上游 1.6.4 的写法而未补足 MT5700
> 私有提示字节的真机差异，叠加一个微妙的串口写入副作用，导致用户每次点发送
> 都收不到 10086 回信。**只允许发 LLCX 到 10086 验证**：能收到 10086 自动回信
> 即视为修复成功。

### 修复 - 短信发送：补齐 Ctrl-Z(0x1A) 终止字节，过滤模组私有提示字节

**问题与证据**：

- 鼎桥《MT5700M-CN 5G 系列模组 AT 命令手册》9.14 节给的样例是
  `AT+CMGS=18\r<PDU>`（**无 0x1A**），v1.0 原样照抄这条命令；
- 真机裸 UART 测（停掉 Rust 后直接 `/dev/ttyUSB1`）只发 `AT+CMGS=15\r`，
  模组在 ~6s 后回 `OK` 自动取消——说明模组实际**等的是 PDU + 0x1A**，
  手册示例是早期固件上恰巧能跑的特例；
- 同一真机测，发送 `AT+CMGS=15\r<PDU>\x1A`（QModem `sms-tool_q` 的写法）
  模组立即回 `+CMGS: <mr>\r\nOK`；
- 后端 `atclient.rs::consume()` 只把 `>` 当作 AT+CMGS 提示字节，没识别 MT5700
  私有约定 SUB(0x1A)；
- 参考实现：
  - `FAN789/luci-app-mt5700m`（同款硬件）通过 `ubus-at-daemon` + `sms-tool_q`
    跑两步提交（`AT+CMGS=N\r` → 等 ~1s → `<PDU>\x1A\r\n`），本质也是要 0x1A；
  - `FUjr/QModem` 的 `sms-tool_q/patches/0001-send_raw_pdu.patch`
    注释里写明 `<pdu>0x1A` 是 PDU 模式的硬性终止符。

**改法（最小变更；功能等价于 QModem 的两步提交）**：

| 文件 | 改前 | 改后 |
| --- | --- | --- |
| `sms_center.js` `send()` 第 377 行 | `'AT+CMGS=' + N + '\r' + pdu` | `'AT+CMGS=' + N + '\r' + pdu + '\u001a'` |
| `atclient.rs::send_command_inner` | `if !cmd.ends_with('\r') { cmd.push('\r'); }` | 同时排除 `'\u{1A}'`，避免在终止字节后再追 CR |
| `atclient.rs::handle_line` | 不过滤 | 把单字节 SUB 行当作模组私有提示字节丢弃，避免污染应答与日志 |

前端发送仍是**一次性**整串命令，但末端的 `\u001a` 让模组立即识别终止并提交；
后端 `is_sms_data_command`（rpcserver.rs 第 725 行）原本就同时认 `AT+CMGS=...`
与「整行 ≥20 个 hex」两步格式，本补丁**不改动**那条路径——给将来「前端两步」
升级留好接口（见下方「未做」）。

**为什么不在前端分两步**：v1.0 的设计是前端把整条命令一次性交给后端，
后端再独立 tokio 任务跑（`SMS_JOB_TIMEOUT=6s`、失败补 ESC、60s 冷却）。
本补丁保留这条管道，仅在**写入模组的字节流**里多出 3 个字节（`\x1A`），
整条链路对外行为不变，前端轮询 `AT+SMSJOB?` 的逻辑零改动。

**真机回归**：

- 修前：发送 `AT+CMGS=15\r<PDU>`（无 0x1A）→ 后端日志
  `模组未确认短信提交：AT+CMGS=15\u001a`（注意末尾的 0x1A 是模组发出的提示字节，
  被原 `consume()` 当成数据攒进去），`waitSmsJob()` 收不到 `+CMGS:`
  → 报「模组未确认短信提交」失败；
- 修后：发送 `AT+CMGS=15\r<PDU>\u001A` → 模组立即回 `+CMGS: <mr>\r\nOK` →
  `waitSmsJob()` 拿到的 `ok=true` → 前端 toast「发送成功」，对方（10086 等）正常
  收到并回信。

### 已记录、本轮不改

- **两步提交未走通**：本补丁是「最小变更」的等价两步修复——前端一次性下发，
  依赖 tty 缓冲让模组原子地消费命令+PDU+终止字节。**理论风险**：极少数 tty
  驱动会在大包写入时拆帧，模组 parser 可能只收到命令部分、PDU 部分掉到下一帧；
  实测本机短短信（≤140 字节，含分片 8B UDH 后整包 < 200B）未触发。
  如果未来出现「长短信偶发失败」的回归，把前端改成显式两步
  （`AT+CMGS=N\r` → `setTimeout(1000)` → `<PDU>\u001A`）即可，
  `is_sms_data_command` 已兼容——见 rpcserver.rs 第 725-740 行。
- **模组的私有提示字节不进入 RPC 应答**：v1.0 修前，`AT+CMGS` 命令的应答里
  会出现一个无法渲染的单字节 SUB 行，污染前端展示与日志。本补丁在
  `handle_line()` 里过滤，但**没有**额外加单元测试——`handle_line` 涉及
  `Arc<AtClient>` 整套结构，新加单测成本不小；接受「真机回归覆盖」作为验收。

### 变更 - 版本号 1.0 → 1.0.1

`Makefile` `PKG_VERSION:=1.0` → `1.0.1`。patch 级递增：本补丁修了 v1.0
就存在的功能故障，不引入新接口或破坏性变更。
