...
本轮是**多端适配**：把散落在各页面的视口媒体查询统一收敛为容器查询体系，
让同一套页面在手机 / 平板 / PC 上按「自身可用宽度」而非「浏览器窗口宽度」排版。
经四角色会审（Proposer 提案 → Builder 实现 → Reviewer 审查 → 主控复核），
汇总 84 条编号 100% 处置；真机 19 档视口 × 触屏/鼠标组合实测 0 溢出 / 0 JS 错误。

### 修改

- **响应式断点改为容器查询**（`mt5700.css`）
  原先各页面各写一套 `@media (max-width:...)`，嵌在窄容器里时窗口再宽也是手机排版。
  统一改为 `@container mt5700`，断点收敛为 **480 / 720 / 960** 三档。
  注：容器查询的条件不支持 CSS 变量，断点只能写死在条件里 —— 全部 8 处位置
  已在文件头部注释中登记（6 处 `@container` + 2 处 `@media`），改断点时按清单改全。

- **栅格单列阈值 480 → 600**
  `.mt5700-grid` 基类是 `minmax(280px, 1fr)`，即设计系统的最小列宽是 280px。
  沿用 480 会让容器 481 时两列各只有 235px，等于破自家下限；600 时两列 294px 才合规。
  影响面已核死：`mt5700-grid-3` / `-4` 全仓零使用。

- **表格横向滚动的渐隐改由滚动容器自身承担**
  原实现是 `::after` 伪元素 + `position:absolute`，它**跟着内容一起滚**，
  实测横滚后渐隐带漂移 122px，等于失效。改为在滚动容器上直接加 `mask-image`
  （mask 作用于元素盒子，不随内容滚动；保留 `-webkit-` 前缀）。

- **终端 / 日志面板高度钳到视口**
  `max-height: min(500px, 55dvh)` / `min(420px, 55dvh)`，手机横屏下不再顶出屏幕。

- **模态弹窗在窄屏改为底部抽屉**，`max-width: min(100%, 640px)`。

- **短信中心的「详情态」收敛为单一写入函数** `setDetail(on)`（`sms_center.js`）
  原先三处各自 `classList.add/remove('is-detail')`，漏一处就是列表详情同屏。

### 移除

- 删除确认已无人使用的死样式族：`.at-toast*` 6 个、`.at-conn*` 9 个及
  `@keyframes at-conn-pulse`。全仓 `.at-*` 类名由 135 降到 29。

### 测试

- `tests/css-cachebust-contract.test.js` 的内容指纹随 CSS 同步更新
  （现 `36438281585f63d7`，80202 字节），防止改了样式却因缓存没生效。
