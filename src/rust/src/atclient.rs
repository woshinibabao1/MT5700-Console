...
impl AtClient {
    ...
    async fn send_command_inner(
        ...
        // 注意：不要在 SUB(0x1A) 终止字节之后再追 '\r'——
        // 短信提交协议是 `AT+CMGS=<N>\r<PDU>\x1A`，末端的 0x1A 是 PDU 的结束
        // 字节，其后再写任何字节都会被部分模组当成"PDU 之后的多余数据"再次
        // 等待下一个 0x1A，导致提交挂死。
        if !cmd.ends_with('\r') && !cmd.ends_with('\u{1A}') {
            cmd.push('\r');
        }
    ...
    async fn handle_line(&self, line: String) {
        // 滤掉模组的 PDU 输入提示字节（SUB = 0x1A）。
        //
        // 3GPP TS 27.005 规定的提示符是 "> "（0x3E），但本项目硬件（MT5700M-CN，
        // 固件 V200R001C20B025）走的是私有约定：AT+CMGS 数据输入阶段单独回送
        // SUB(0x1A) 作为提示。它不是应答内容，留下来会污染前端显示与日志；
        // 单字节 0x1A 行也无法被前端按"行"渲染（控制字符），必须滤掉。
        if line.as_bytes() == [0x1A] {
            return;
        }
        if line.is_empty() {
            return;
